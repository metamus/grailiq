...
if (priceWorker) {
  ...
  priceWorker.on('completed', async (job, result) => {
    logger.info(
      {
        jobId: job.id,
        tier: job.data?.tier,
    ...
    // Kick off a score recalc for "hot" tier updates — fresh prices deserve
    // fresh scores and signals. Non-blocking enqueue.
    if (job.data?.tier === 'hot' && scoreQueue) {
      try {
        await scoreQueue.add(
          'recalculate-after-hot-prices',
          { triggeredBy: 'price-update', jobId: job.id },
          { removeOnComplete: 50, removeOnFail: 20 }
        );
        logger.info({ jobId: job.id }, 'Enqueued post-price score recalculation');
      } catch (err) {
        logger.warn({ err }, 'Failed to enqueue post-price score recalc');
      }
    }
  });
